...
def IsingSolve(nX, nY, nZ, h, J, q):
	...
	network.contract(compressL=True,mergeL=True,eps=1e-5)

	for i in list(network.topLevelNodes()):
		print(i.tensor().array(),i.logScalar())

	return np.log(list(network.topLevelNodes())[0].tensor().array()) + list(network.topLevelNodes())[0].logScalar()

#print IsingSolve(10,10,10,0,0.5)/1000
#print IsingSolve(10,10,0,0.5)/100
print(otherIsingSolve(12,12,12,-0.2,-0.2,1.))
