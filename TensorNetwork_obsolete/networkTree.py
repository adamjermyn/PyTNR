...
class NetworkTree:
    ...
    def largestTensor(self):
        ...
        '''
        def sizeGetter(n): return n.tensor().size()
    ...
    def addNodeFromArray(self, arr):
        ...
        return Node(t, self, Buckets=[Bucket(self)
    ...
    def filterSubset(self, subset):
        ...
        while len(nn.nodes().intersection(subset)) < len(subset):
            ...
            for n in nn.nodes():
                for b in n.buckets():
                    assert len(
                        set(b.otherNodes()).intersection(nn.nodes())) == 1
    ...
    def contract(self, mergeL=True, compressL=True, eps=1e-4):
        ...
        while self._sortedLinks.length > 0:
            ...
            if counter % 1 == 0:
                ...
                print(len(self.topLevelNodes()),
                      self.topLevelSize(), t.tensor().shape())
            counter += 1
                    ...
                    if b.linked():
                        ...
                        if b.numNodes() == 1 and numC == 1:
                            ...
                            if link.compressed() and not link.optimized():
                                ...
                                _, arr, bs = self.view(
                                    [n11, n22], mergeL=mergeL, compressL=compressL, eps=eps)
                                ...
                                newLink, n1, n2 = compress(
                                    prevLink, optimizerArray=arr, optimizerBuckets=bs, eps=eps)

                                self.contract(
                                    mergeL=mergeL, compressL=compressL, eps=eps)
