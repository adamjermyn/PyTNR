...
def shortestPaths(sp, n):
    ...
    dists, predecessors = shortest_path(
        sp, method='D', directed=False, return_predecessors=True, unweighted=False, overwrite=False)
...
def shortestPathsAlt(g, n):
    ...
    for j in range(n):
        length = 1e100
        length, path = networkx.single_source_dijkstra(
            g, j, target=j + n, weight='weight', cutoff=best[0])
        if j + n in path.keys():
            length = length[j + n]
            path = path[j + n]
            if length < best[0]:
...
class traceMin:
        ...
        for n in self.otherNodes:
            if hasattr(n.tensor, 'compressedSize'):
                for b in n.tensor.network.externalBuckets:
                    if b.linked:
                        u.add_edge(
                            b.node, b.otherNode, weight=np.log(
                                b.node.tensor.size * b.otherNode.tensor.size))
    ...
    def swapBenefit(self, g, basis, edge, b1, b2):
        ...
        logger.debug('Computing swap benefit across ' +
                     str(len(g.nodes())) +
                     ' nodes with respect to basis with ' +
                     str(len(nodes)) +
                     ' nodes and ' +
                     str(len(self.diffVals)) +
                     ' cached values.')
...
#            return self.diffVals[cacheSet]
    ...
    def bestSwap(self):
        ...
        for e in gNew.edges():
            if e in self.selfGraph.edges() or (
                    e[1], e[0]) in self.selfGraph.edges():
                n1 = e[0]
                n2 = e[1]
                l = n1.findLink(n2)
    ...
    def mergeSmall(self):
        ...
        while len(
                self.network.nodes.intersection(done)) < len(
                self.network.nodes):
            n1 = next(iter(self.network.nodes.difference(done)))

            merged = False
            for n2 in n1.connectedNodes:
                if not merged and (len(n2.buckets) < 3 or len(n1.connectedNodes.intersection(
                        n2.connectedNodes)) > 0 or len(n1.findLinks(n2)) > 1):
                    self.network.mergeNodes(n1, n2)
                    merged = True
                    mergedAny = True
    ...
    def swap(self, edge, b1, b2):
        ...
        nodes = self.network.splitNode(
            n, ignore=[n.bucketIndex(b1), n.bucketIndex(b2)])
