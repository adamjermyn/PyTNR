...
def mergeLinks(n1, n2, compressLink=False, eps=1e-4):
    ...
    arr1m = np.reshape(arr1m, list(arr1m.shape[:m1]) + [np.product(
        [arr1.shape[i] for i in indices1])] + list(arr1m.shape[m1 + len(indices1):]))
    arr2m = np.reshape(arr2m, list(arr2m.shape[:m2]) + [np.product(
        [arr2.shape[i] for i in indices2])] + list(arr2m.shape[m2 + len(indices2):]))
    ...
    newLink = n1m.addLink(n2m, m1, m2, compressed=False, children=links)

    if compressLink:
        link = n1m.findLink(n2m)
        _, n1m, n2m = compress(link, eps=eps)

    return n1m, n2m, newLink
