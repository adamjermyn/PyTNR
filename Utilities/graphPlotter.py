...
def makePlotter(fname, counter=0, positions=None):
	...
	def plotter(graph):
		...
		if positions is not None:
			...
			pos = networkx.fruchterman_reingold_layout(graph, pos=reusePos, fixed=reusePos.keys())
		else:
			pos = networkx.fruchterman_reingold_layout(graph)

		plt.figure(figsize=(11,11))
		weights = [graph.edge[i][j]['weight']**2/5 for (i,j) in graph.edges_iter()]
		networkx.draw(graph, pos, width=weights, edge_color=[cm.jet(w/max(weights)) for w in weights])
