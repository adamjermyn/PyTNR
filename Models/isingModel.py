...
def IsingModel2Dopen(nX, nY, h, J, accuracy): 
	...
	for i in range(nX):
		for j in range(nY):
			...
			if j > 0:
				Link(lattice[i][j].buckets[counters[i][j]], bondV[i][j-1].buckets[0])
				counters[i][j] += 1
			if j < nY - 1:
				print(counters[i][j])
				Link(lattice[i][j].buckets[counters[i][j]], bondV[i][j].buckets[1])
				counters[i][j] += 1
			if i > 0:
				print(counters[i][j])
				Link(lattice[i][j].buckets[counters[i][j]], bondH[i-1][j].buckets[0])
				counters[i][j] += 1
			if i < nX - 1:
				print(counters[i][j])
				Link(lattice[i][j].buckets[counters[i][j]], bondH[i][j].buckets[1])
				counters[i][j] += 1
...
def IsingSpinGlass(n, J, k, accuracy): 
	...
	choices = np.random.choice(list(range(len(options))), size=nBonds, replace=False)
	choices = [options[c] for c in choices]

	ranks = [sum([1 for i,j in options if i == q or j == q]) for q in range(n)]
